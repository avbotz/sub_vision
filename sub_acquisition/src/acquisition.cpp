...
#include "sub_acquisition/acquisition.hpp"
    ...
    try
    {
        ...
        if (!IsAvailable(acq_mode) || !IsWritable(acq_mode))
        {
            RCLCPP_ERROR(rclcpp::get_logger("vision_acquisition_node"),
                    "Camera is either unavailable and/or unwritable");
            return;
        }
        ...
        {
            RCLCPP_ERROR(rclcpp::get_logger("vision_acquisition_node"),
                    "Camera cannot be set to continuous.");
            return;
        }
    }
...
void setupContinuousAcquisition(CameraPtr camera, int exposure_time)
{
    try
    {
        ...
        {
            RCLCPP_ERROR(rclcpp::get_logger("vision_acquisition_node"),
                    "camera cannot be set continuous.");
            return;
        }
